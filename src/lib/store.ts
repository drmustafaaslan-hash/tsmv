...
export const useStore = create<StoreState>()((set, get) => ({
  ...
  fetchAll: async () => {
    ...
    try {
      // Members
      const { data: membersData, error: membersError } = await supabase.from('members').select('*').order('no');
      if (membersError) console.error('Members hatası:', membersError);
      ...
      if (profilesError) console.error('Profiles hatası:', profilesError);
      ...
      if (donationsError) console.error('Donations hatası:', donationsError);
      ...
      // Pending profiles
      const { data: pendingProfilesData, error: pendingProfilesError } = await supabase.from('pending_profiles').select('*').eq('status', 'pending');
      if (pendingProfilesError) console.error('Pending profiles hatası:', pendingProfilesError);
      ...
      // Pending donations
      const { data: pendingDonationsData, error: pendingDonationsError } = await supabase.from('pending_donations').select('*').eq('status', 'pending');
      if (pendingDonationsError) console.error('Pending donations hatası:', pendingDonationsError);
      ...
      if (settingsError && settingsError.code !== 'PGRST116') console.error('Settings hatası:', settingsError);
